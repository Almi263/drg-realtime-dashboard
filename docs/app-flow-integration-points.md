...
| Document view/download | Checks Entra role and active `drg_programaccess`, creates successful `drg_documentaccesslog` row with action `View` or `Download`, fetches the file through the app's Microsoft Graph SharePoint credential, and streams it back to the browser | Review-start/viewed flows update document/deliverable status and viewed fields only for successful Web App access events |
